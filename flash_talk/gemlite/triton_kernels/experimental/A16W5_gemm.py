...
def gemm_A16fWnO16f_int32packing_kernel(
    ...
    N,
    K,
    W_nbits_1: tl.constexpr,
    W_nbits_2: tl.constexpr,
    group_size: tl.constexpr,
    unpack_mask_1: tl.constexpr,
    unpack_mask_2: tl.constexpr,
    elements_per_sample_1: tl.constexpr,
    elements_per_sample_2: tl.constexpr,
    stride_am,
    stride_ak,
    stride_bk_1,
    stride_bn_1,
    ...
    b_ptrs_1 = b_1_ptr + (
        (offs_k[:, None] // elements_per_sample_1) * stride_bk_1
        + offs_bn[None, :] * stride_bn_1
    )
    b_ptrs_2 = b_2_ptr + (
        (offs_k[:, None] // elements_per_sample_2) * stride_bk_2
        + offs_bn[None, :] * stride_bn_2
    )
    ...
    for k in tl.range(0, num_pid_k, 1, num_stages=1):
        b1 = (
            (tl.load(b_ptrs_1, eviction_policy="evict_first") >> q_shifts1)
            & unpack_mask_1
        ).to(tl.uint8)  # (BLOCK_SIZE_K, BLOCK_SIZE_N) - repeated over K dim
        b2 = (
            (tl.load(b_ptrs_2, eviction_policy="evict_first") >> q_shifts2)
            & unpack_mask_2
        ).to(tl.uint8)  # (BLOCK_SIZE_K, BLOCK_SIZE_N) - repeated over K dim
...
def gemm_A16fWnO16f_int32packing_forward(
    ...
    unpack_mask_1,
    unpack_mask_2,
    elements_per_sample_1,
    elements_per_sample_2,
    acc_dtype=tl.float16,
):
    output = torch.empty(
        (x.shape[0], W_q_1bit.shape[1]), device=W_q_1bit.device, dtype=scales.dtype
    )
    ...
    grid = lambda META: (
        ...
        * triton.cdiv(W_q_1bit.shape[1], META["BLOCK_SIZE_N"]),
    )
...
class GemliteLinear(torch.nn.Module):
    def __init__(self, W_nbits, group_size, in_features, out_features, bias):
    ...
    def pack_base(self, W_q, n_bits):
        ...
        W_q_out = torch.zeros(
            (W_q.shape[0] // 32 * n_bits, W_q.shape[1]),
            dtype=torch.int32,
            device=W_q.device,
        )  # Packed
    ...
    def forward(self, x):
        ...
        out = gemm_A16fWnO16f_int32packing_forward(
            ...
            self.unpack_mask_1,
            self.unpack_mask_2,
            self.elements_per_sample_1,
            self.elements_per_sample_2,
        ).reshape(out_shape)
...
def matmul_torch_A16W8SYM(x, W_q, scales, out_features):
    ...
    out = ((x.view((-1, x.shape[-1])) @ W_q.T.to(x.dtype)) / scales.view(1, -1)).view(
        out_shape
    )
    return out
...
class Torch_A16W8SYM(torch.nn.Module):
    ...
    def forward(self, x):
        out = matmul_torch_A16W8SYM(
            x.to(self.device), self.W_q, self.scales, self.out_features
        )
        if self.bias is not None:
            out += self.bias
        return out
...
def eval_time(fct, params):
    return do_bench(
        lambda: fct(**params), warmup=25, rep=200, fast_flush=True, return_mode="min"
    )
...
def gen_data(in_features, out_features, W_nbits, group_size, device="cuda:0"):
    linear = torch.nn.Linear(
        in_features=in_features, out_features=out_features, bias=False, device="cpu"
    )
    ...
    W_q = torch.randint(
        0, 2**W_nbits, (out_features, in_features), dtype=torch.uint8, device=device
    ).to(torch.uint8)
    ...
    scales = torch.randn((N,), dtype=compute_dtype, device=device).abs() / 500.0
    ...
    W = (
        (W_q.reshape([-1, group_size]) - zeros.view((N, 1))) * scales.view((N, 1))
    ).reshape(orig_shape)
    ...
    if W_nbits in [5]:
        ...
        triton_linear = GemliteLinear(
            W_nbits=W_nbits,
            group_size=group_size,
            in_features=in_features,
            out_features=out_features,
            bias=False,
        ).pack(W_q, scales, zeros)
...
W, triton_linear, torchao_linear, bitblas_linear, marlin_linear = gen_data(
    in_features, out_features, W_nbits, group_size
)
...
for batch_size in BATCH_SIZES:
    x = (
        torch.randn(
            ...
            dtype=triton_linear.compute_dtype,
            device="cuda:0",
        )
        / 10.0
    )
    ...
    if triton_linear is not None:
        triton_time = eval_time(
            lambda x: triton_linear(x), {"x": x.to(triton_linear.compute_dtype)}
        )
        print(
            (batch_size, in_features, out_features),
            "Triton  Speed-up vs. torch.matmul",
            np.round(ref_time / triton_time, 2),
            "time",
            triton_time,
        )

    if torchao_linear is not None:
        torchao_time = eval_time(
            lambda x: torchao_linear(x), {"x": x.to(torchao_linear.compute_dtype)}
        )
        print(
            (batch_size, in_features, out_features),
            "Torchao Speed-up vs. torch.matmul",
            np.round(ref_time / torchao_time, 2),
        )

    if bitblas_linear is not None:
        bitblas_time = eval_time(
            lambda x: bitblas_linear(x), {"x": x.to(bitblas_linear.compute_dtype)}
        )
        print(
            (batch_size, in_features, out_features),
            "Bitblas Speed-up vs. torch.matmul",
            np.round(ref_time / bitblas_time, 2),
        )

    if marlin_linear is not None:
        marlin_time = eval_time(
            lambda x: marlin_linear.forward(x), {"x": x.to(marlin_linear.compute_dtype)}
        )
        print(
            (batch_size, in_features, out_features),
            "Marlin Speed-up vs. torch.matmul",
            np.round(ref_time / marlin_time, 2),
        )
