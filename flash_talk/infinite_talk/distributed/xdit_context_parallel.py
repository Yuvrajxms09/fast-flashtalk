...
def usp_dit_forward_vace(self, x, vace_context, seq_len, kwargs):
    ...
    # Context Parallel
    c = torch.chunk(c, get_sequence_parallel_world_size(), dim=1)[
        get_sequence_parallel_rank()
    ]
...
def usp_attn_forward_multitalk(
    self, x, seq_lens, grid_sizes, freqs, dtype=torch.bfloat16, ref_target_masks=None
):
    b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
    half_dtypes = (torch.float16, torch.bfloat16)
...
def usp_crossattn_multi_forward_multitalk(
    ...
    visual_seqlen, frame_ids = split_token_counts_and_frame_ids(
        N_t, N_h * N_w, sp_size, sp_rank
    )
    encoder_hidden_states = encoder_hidden_states[
        :, min(frame_ids) : max(frame_ids) + 1, ...
    ]
    ...
    back = torch.full((x_ref_attn_map.size(1),), self.rope_bak, dtype=human1.dtype).to(
        human1.device
    )
    ...
    per_frame = torch.zeros(
        audio_tokens_per_frame * human_num, dtype=encoder_k.dtype
    ).to(encoder_k.device)
