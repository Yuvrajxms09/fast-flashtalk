...
def kernel_config_pruner(configs, nargs, **kwargs):
    ...
    if MATMUL_TYPE in GEMLITE_TRITON_CONFIG_CACHE:
        ...
        if signature in GEMLITE_TRITON_CONFIG_CACHE[MATMUL_TYPE]:
            ...
            yield triton.Config(
                config, num_stages=num_stages, num_warps=num_warps, pre_hook=pre_hook
            )
            return
    ...
    for config in configs:
        ...
        used.add(key)
        yield triton.Config(
            new_config, num_stages=num_stages, num_warps=num_warps, pre_hook=pre_hook
        )
...
def gemv_INT_kernel(
    ...
    if W_group_mode >= 2:  # [2, 3, 4]
        scales = tl.load(
            scales_ptr + k_m * stride_meta_g + offs_bn[None, :] * stride_meta_n,
    ...
    if W_group_mode == 1 or W_group_mode >= 3:  # [1, 3, 4]
        if zero_is_scalar:
            ...
            zeros = tl.load(
                zeros_ptr + k_m * stride_meta_g + offs_bn[None, :] * stride_meta_n,
                eviction_policy=meta_evict_policy,
            )
    else:
        zeros = None
    ...
    if join_version:
        if elements_per_sample == 2:
            b = (
                tl.join(b, b)
                .permute(0, 2, 1)
                .reshape((BLOCK_SIZE_K, BLOCK_SIZE_N), can_reorder=False)
            )
        ...
        if elements_per_sample == 8:
            b = (
                tl.join(b, b)
                .permute(0, 2, 1)
                .reshape((BLOCK_SIZE_K // 4, BLOCK_SIZE_N), can_reorder=False)
            )
            b = (
                tl.join(b, b)
                .permute(0, 2, 1)
                .reshape((BLOCK_SIZE_K // 2, BLOCK_SIZE_N), can_reorder=False)
            )
            b = (
                tl.join(b, b)
                .permute(0, 2, 1)
    ...
    if dot_prod_mode == 0:
        acc = tl.sum(
            a.reshape((BLOCK_SIZE_K, 1), can_reorder=False).to(acc_dtype)
            * b.to(acc_dtype),
            axis=0,
            keep_dims=True,
        )
    if dot_prod_mode == 1:
        acc = tl.sum(
            a.reshape((BLOCK_SIZE_K, 1), can_reorder=False) * b.to(input_dtype),
            axis=0,
            keep_dims=True,
        )
...
def gemv_MX_kernel(
    ...
    a_mask = (
        (offs_am[:, None] < M) & (offs_ak[None, :] < (K // elements_per_sample_a))
    ).to(tl.int1)

    if W_nbits == 4:  # mxpf4 mapping
        mapping = tl.load(mapping_ptr + tl.arange(0, 16), eviction_policy="evict_last")[
            None, :
        ].broadcast_to((BLOCK_SIZE_K, 16))

    ###################################################################
    # Load
    if A_load_order == 0:
        a = tl.load(a_ptrs, mask=a_mask, other=0.0, eviction_policy=a_evict)
    ...
    if channel_scale_mode == 4:
        scales_a_ptrs = (
            scales_a_ptr + k_m * stride_meta_a_g + offs_am[None, :] * stride_meta_a_m
        )
    ...
    a = a.reshape(
        (BLOCK_SIZE_K, 1), can_reorder=False
    )  # we work with transposed activations
...
def gemv_forward(
    ...
    native_atomic = (
        output_dtype in [DType.FP16.value, DType.FP32.value]
    ) or NATIVE_ATOMIC
    kernel_output_dtype = (
        DTYPE_TO_TORCH[output_dtype] if native_atomic else torch.float32
    )
    ...
    if KERNEL.ENABLE_CACHING and M == 1:
        ...
        output, use_prehook = (
            torch.zeros((M, N), device=W_q.device, dtype=kernel_output_dtype),
            False,
        )
    ...
    gemv_kernel[grid](
        ...
        dump_b_val=0.001
        if (W_group_mode in [0, 1] and acc_dtype == DType.FP16.value and W_nbits == 8)
        else 0,  # Warning: Only use with INT8
    )
