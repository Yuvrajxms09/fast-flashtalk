                ...
                for s in [1, 2]:
                    for M in [1]:  # ONLY 1 allowed here
                        for N in [
                            ...
                            256,
                            512,
                        ]:  # contiguous: [128, 256, 512] / non-contiguous: [16, 32, 64, 128, 256, 512, 1024, 2048]
                            for K in [
                                8,
                                16,
                                32,
                                64,
                            ]:  # contiguous: [8, 16, 32, 64] / non-contiguous: [8, 16, 32, 64, 128, 256, 512, 1024, 2048]
                                configs.append(
                                    triton.Config(
                                        {
                                            "BLOCK_SIZE_M": M,
...
def gemv_INT_revsplitK_kernel(
    ...
    if W_group_mode >= 2:  # [2, 3, 4]
        scales = tl.load(
            scales_ptr + offs_bn[None, :] * stride_meta_n + k_m * stride_meta_g,
    ...
    if W_group_mode == 1 or W_group_mode >= 3:  # [1, 3, 4]
        if zero_is_scalar:
            ...
            zeros = tl.load(
                zeros_ptr + offs_bn[None, :] * stride_meta_n + k_m * stride_meta_g,
                eviction_policy=meta_evict_policy,
            )
    else:
        zeros = None
    ...
    # Load
    if A_load_order == 0:
        a = tl.load(a_ptrs, eviction_policy=a_evict).reshape(
...
def gemv_revsplitK_forward(
    ...
    grid = lambda meta: (
        ...
        triton.cdiv(K, meta["BLOCK_SIZE_K"] * 2),
    )
    ...
    gemv_revsplitK_kernel[grid](
        ...
        dump_b_val=0.001
        if (W_group_mode in [0, 1] and acc_dtype in [DType.FP16.value] and W_nbits == 8)
