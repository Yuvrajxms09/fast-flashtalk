...
from .core import GemLiteLinearTriton, DType, TORCH_TO_DTYPE
from .triton_kernels.utils import M_MAPPING
            ...
            if hasattr(processor, "from_hqqlinear"):
                if isinstance(layer, torch.nn.Linear):
                    ...
                    quant_config = BaseQuantizeConfig(
                        nbits=W_nbits, group_size=group_size if W_nbits <= 4 else None
                    )
                    layer = HQQLinear(
                        layer,
...
class A16W8:  # INT8/FP8 weight-only channel-wise
    def __init__(
        self, device="cuda:0", dtype=None, fp8=None, fp32_scale=True, post_scale=False
    ):
        self.device = device
        self.dtype = dtype
        self.fp8 = fp8
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        if scales is None:
            # Quantize
            if self.fp8:  # FP8
                w_dtype, min_val, max_val = (
                    self.fp8,
                    torch.finfo(self.fp8).min,
                    torch.finfo(self.fp8).max,
                )
            else:  # INT8
                w_dtype, min_val, max_val = (
                    torch.int8,
            ...
            # Pre-Quantized
            assert weight.itemsize == 1, (
                f"Invalid weight.dtype, should be 8-bit (INT8 or FP8), got {weight.dtype}"
            )
            if self.dtype is None:
                dtype = (
                    scales.dtype
                    if scales.dtype in [torch.float16, torch.bfloat16]
                    else torch.float16
                )
            else:
                dtype = self.dtype
    ...
    def from_linear(self, linear_layer, del_orig=True):
        out_layer = self.from_weights(
            weight=linear_layer.weight, bias=linear_layer.bias
        )
...
class A16Wn:  # 8/4/2-bit weight-only as grouped "INT" / 8/4-bit as MXFP type
    ...
    def from_weights(
        self, W_q, scales, zeros, W_nbits, group_size, bias=None, quant_type="INT"
    ):
        return self.from_weights_(
            W_q, scales, zeros, W_nbits, group_size, bias, quant_type
        )

    def from_weights_(
        self, W_q, scales, zeros, W_nbits, group_size, bias=None, quant_type="INT"
    ):
        assert quant_type in ["INT", "MXFP"], (
            f"Invalid quant_type. Got {quant_type}, valid values are INT, MXFP."
        )
        ...
        if quant_type == "INT":
            ...
            if W_nbits == 8:
                assert W_q.dtype in [self.mx_fp8_dtype], (
                    f"Unsupported dtype of W_q. got {W_q.dtype}"
                )
            if W_nbits == 4:
                assert W_q.dtype in [torch.uint8], (
                    f"Unsupported dtype of W_q. got {W_q.dtype}"
                )
            ...
            if dtype == torch.float16:
                ...
                raise Exception(
                    f"Unsupported dtype for MXFP. Got {dtype}, supported [torch.float16, torch.bfloat16]"
                )
    ...
    def mxfp_from_linear(self, linear_layer, W_nbits, del_orig=True):
        if self.quantizer_mx is None:
            self.quantizer_mx = WeightQuantizerMXFP(
                device=self.device, compute_dtype=linear_layer.weight.dtype
            )
        ...
        N, K = W.shape
        if W_nbits == 8:
            W_q, scales = self.quantizer_mx.quantize_mxfp8(
                W, index=True, mx_fp8_dtype=self.mx_fp8_dtype
            )
        if W_nbits == 4:
...
class A8W8_dynamic:
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        if self.fp8:  # FP8
            w_dtype, input_dtype, min_val, max_val = (
                self.fp8,
                TORCH_TO_DTYPE[self.fp8],
                torch.finfo(self.fp8).min,
                torch.finfo(self.fp8).max,
            )
        else:  # INT8
...
class A8Wn_HQQ_INT_dynamic(A16Wn):
    def __init__(
        ...
        assert W_nbits is not None, (
            "W_nbits argument should be eitehr 8 or 4, not None)."
        )
        super().__init__()
        self.post_scale = post_scale
        self.device = device
    ...
    def from_weights_(self, W_q, scales, zeros, W_nbits, group_size, bias=None):
        ...
        assert dtype in [torch.float16, torch.bfloat16, torch.float32], (
            "Invalid scales.dtype, should be floating point."
        )
        gemlite_dtype = TORCH_TO_DTYPE[dtype]
        ...
        scales = (
            scales.to(
                device=self.device, dtype=torch.float32 if self.fp32_scale else dtype
            )
            if (scales is not None)
            else None
        )
        zeros = (
            zeros.to(device=self.device, dtype=dtype) if (zeros is not None) else None
        )
        bias = bias.to(device=self.device, dtype=dtype) if (bias is not None) else None
    ...
    def from_hqqlinear(self, hqq_layer, del_orig=True):
        ...
        W_q = hqq_layer.unpack(dtype=torch.uint8).view(
            hqq_layer.meta["shape"]
        )  # Expects uint8 for Wn quantization!
        scales = hqq_layer.meta["scale"].clone()
        zeros = hqq_layer.meta["zero"].clone()
        bias = hqq_layer.bias.clone() if (hqq_layer.bias is not None) else None
...
class A8W8_MXFP_dynamic:
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        assert scales is not None, (
            "Scales parameter cannot be None. Use from_linear() call to pre-quantize the weights."
        )

        # Pre-Quantized
        assert weight.is_floating_point() and weight.itemsize == 1, (
            f"Invalid weight.dtype, should be an MXPF8 dype, got {weight.dtype}."
        )
        assert scales.dtype in [torch.float8_e8m0fnu, torch.uint8], (
            f"Invalid scales.dtype, should be e8m0 / view(uint8), got {scales.dtype}."
        )
        assert self.dtype is not None, (
            "Input dtype should be either torch.float16 or torch.bfloat16, not None."
        )
        dtype = self.dtype
...
class A8Wn_MXFP_dynamic:
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        if self.W_nbits == 8:
            assert weight.is_floating_point() and weight.itemsize == 1, (
                f"Invalid weight.dtype, should be an MXPF8 valid dtype, got {weight.dtype}."
            )
        if self.W_nbits == 4:
            assert weight.dtype in [torch.uint8], (
                f"Invalid weight.dtype, should be an MXPF8 (FP8 dtype) valid dtype, got {weight.dtype}."
            )
        assert scales.dtype in [torch.float8_e8m0fnu, torch.uint8], (
            f"Invalid scales.dtype, should be e8m0 / view(uint8), got {scales.dtype}."
        )
...
class A8W8_MXFP_dynamic(A8Wn_MXFP_dynamic):
    def __init__(self, device="cuda:0", dtype=None, post_scale=True, fp8=default_fp8):
        super().__init__(
            device=device, dtype=dtype, post_scale=post_scale, fp8=fp8, W_nbits=8
        )
...
class A8W4_MXFP_dynamic(A8Wn_MXFP_dynamic):
    def __init__(self, device="cuda:0", dtype=None, post_scale=True, fp8=default_fp8):
        super().__init__(
            device=device, dtype=dtype, post_scale=post_scale, fp8=fp8, W_nbits=4
        )
...
class A4W4_MXFP_dynamic:
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        assert self.group_size == 32, (
            f"Only group_size=16 is supported for MXFP4, got {self.group_size}"
        )
...
class A4W4_NVFP_dynamic:
    ...
    def from_weights(self, weight, bias=None, scales=None):
        ...
        # Pre-Quantized
        assert weight.dtype in [torch.uint8], (
            f"Invalid weight.dtype, should be an MXPF8 dtype, got {weight.dtype}."
        )
        assert scales.dtype in [torch.float8_e4m3fn], (
            f"Invalid scales.dtype, should be float8_e4m3fn, got {scales.dtype}."
        )
        assert self.dtype is not None, (
            "Input dtype should be either torch.float16 or torch.bfloat16, not None."
        )
        assert self.group_size == 16, (
            f"Only group_size=16 is supported for NVFP4, got {self.group_size}"
        )
...
class A16W158_INT:
    ...
    def from_weights(self, weight, weight_scale, bias=None):
        ...
        scales = (
            torch.ones((out_features, 1), dtype=torch.float32) * weight_scale.item()
        )
...
class A8W158_INT_dynamic:
    ...
    def from_weights(self, weight, weight_scale, bias=None):
        ...
        in_features = W_q.shape[1]
        scales = (
            torch.ones((out_features, 1), dtype=torch.float32, device=self.device)
            * weight_scale.item()
        )
...
def warmup(
    ...
    for shape in shapes:
        ...
        if hasattr(processor, "from_hqqlinear"):
            ...
            linear = HQQLinear(
                linear, quant_config=quant_config, compute_dtype=dtype, device=device
            )
        ...
        for batch_size in tqdm(batch_sizes):
            _ = gemlite_linear(
                torch.randn((batch_size, in_features), dtype=dtype, device=device)
                / 100.0
            )
