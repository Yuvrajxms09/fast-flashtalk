...
GEMLITE_TRITON_MAPPING = {
    kernel.matmul_type: kernel for kernel in GEMLITE_TRITON_KERNELS
}
GEMLITE_MATMUL_TYPES = [kernel.matmul_type for kernel in GEMLITE_TRITON_KERNELS]
GEMLITE_MATMUL_TYPES_MAPPING = {
    GEMLITE_MATMUL_TYPES[i]: i for i in range(len(GEMLITE_MATMUL_TYPES))
}
...
def set_acc_dtype(dtype):
    ...
    assert dtype in [DType.FP16, DType.FP32], (
        "Invalid dtype (should be DType.FP16 or DType.FP32)."
    )
...
def forward_functional(
    ...
    input_dtype = (
        DType.MXFP16.value if (input_dtype == DType.MXBF16.value) else input_dtype
    )
    ...
    if scaled_activations:
        ...
        if input_dtype in FP8_INT8_DTYPES:  # INT8 / FP8
            x, scales_x = scale_activations_per_token(
                x, w_dtype=DTYPE_TO_TORCH[input_dtype.value]
            )
            ...
            input_dtype in [DType.MXFP8] and channel_scale_mode == 2
        ):  # MXPF8 with post-scale for the activations
    ...
    if matmul_type >= 0:
        ...
        matmul_type_str = get_matmul_type(
            x.shape[0], W_nbits, is_mx_dtype(input_dtype)
        )  # batch_size, W_nbits, is_mx_dtype

    out = (
        GEMLITE_TRITON_MAPPING[matmul_type_str]
        .forward(x, *tensor_args, scales_x, *meta_args[1:-1], data_contiguous, type_id)
        .view(out_shape)
    )
...
class GemLiteLinearTriton(torch.nn.Module):
    ...
    def __init__(
        ...
        # Accumulation
        self.acc_dtype = (
            GEMLITE_ACC_DTYPE[self.input_dtype] if (acc_dtype is None) else acc_dtype
        )
    ...
    def pack(
        ...
        self.meta_is_channelwise = (
            False if (self.scales is None) else self.scales.numel() == self.out_features
        )
        ...
        if zeros is None:
            ...
            if isinstance(zeros, torch.Tensor):
                if fma_mode and (
                    self.meta_is_channelwise is False
                ):  # W ~ Wq * scales + zeros
                    self.zeros = (
                        (-zeros.float() * scales.float())
                        .to(zeros.dtype)
        ...
        if (self.scaled_activations == False) and (self.meta_is_channelwise == True):
            ...
            self.W_group_mode = (
                1 if (self.zeros is not None) else 0
            )  # only with fma_mode=False
        ...
        if self.channel_scale_mode in [1, 3]:
            assert self.W_group_mode not in [3, 4], (
                "Can't use channel_scale_mode with W_group_mode == 3 or 4."
            )
        ...
        self.bias = (
            torch.nn.Parameter(self.bias, requires_grad=False)
