...
def kernel_config_pruner(configs, nargs, **kwargs):
    ...
    for config in configs:
        ...
        if m <= 16:
            ...
            block_size_m = min(max(block_size_m, 16), 32)  # m: [16...32]
        elif m <= 64:
            block_size_m = min(max(block_size_m, 32), 64)  # m: [32...64]
        elif m <= 128:
            block_size_m = min(max(block_size_m, 64), 128)  # m: [64...128]
        elif m <= 256:
            block_size_m = min(max(block_size_m, 64), 256)  # m: [128...256]
        elif m > 256:
            block_size_m = min(max(block_size_m, 64), 256)  # m > 256

        # Constraint: BLOCK_SIZE_K >= group_size, only for load_as_block = False
        if load_scales_as_block:
            num_stages = max(
                num_stages, 2
            )  # for dot_scaled kernels with pipelined loads
            if e > 1:
        ...
        while num_stages > 0 and not load_scales_as_block:  # TODO: revisit MXFP case
            shared_mem = (
                block_size_m * block_size_k * a_sizeof
        ...
        if IS_HIP:
            ...
            new_config["matrix_instr_nonkdim"] = config.kwargs.get(
...
def gemm_MX_kernel(
    ...
    scales_b_ptrs = (
        scales_ptr
        + offs_n_b_scales[:, None] * stride_meta_n
        + offs_k_scales[None, :] * stride_meta_g
    )  # [BLOCK_SIZE_N, BLOCK_SIZE_K // group_size]

    # B-scales
    if channel_scale_mode == 4:
        scales_a_ptrs = (
            scales_a_ptr
            + offs_am[:, None] * stride_meta_a_m
            + offs_k_scales[None, :] * stride_meta_a_g
        )
    ...
    for k in tl.range(num_pid_k, num_stages=NUM_STAGES):
        ...
        scales_b = tl.load(
            scales_b_ptrs + k_m * stride_meta_g, eviction_policy=meta_evict_policy
        )

        if channel_scale_mode == 4:
            scales_a = tl.load(
                scales_a_ptrs + k_m * stride_meta_a_g, eviction_policy=meta_evict_policy
            )
        else:
            scales_a = tl.full(
                (BLOCK_SIZE_M, BLOCK_SIZE_K_S), value=127, dtype=tl.uint8
            )
