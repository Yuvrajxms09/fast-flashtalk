...
def kernel_config_pruner(configs, nargs, **kwargs):
    ...
    for config in configs:
        ...
        if not IS_HIP:
            if e > 1:
                num_stages = min(num_stages, 4)  # Limit num stages when data is packed
            if e == 1 and num_stages == 1:
                continue  # skip num_stages=1 for non-packed weights
...
def gemm_splitK_persistent_kernel(
    ...
    for tile_id in range(pid, num_tiles, num_sms):
        pid_m, pid_n = swizzle_tile_persistent(
        ...
        if data_contiguous:
            ...
            offs_bn = tl.max_contiguous(
                tl.multiple_of(offs_n, BLOCK_SIZE_N), BLOCK_SIZE_N
            )
        ...
        for k in range(0, num_pid_k):
            ...
            if elements_per_sample > 1:
                q_shift = ((offs_ak % elements_per_sample) * W_nbits).to(tl.int32)[
                    :, None
                ]
                b = dequantize(
                    b,
...
def gemm_splitK_persistent_forward(
    x: Tensor,
    W_q: Tensor,
    scales: Tensor,
    zeros: Tensor,
