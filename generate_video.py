...
def _validate_args(args):
    ...
    assert args.ckpt_dir is not None, (
        "Please specify FlashTalk model checkpoint directory."
    )
    assert args.wav2vec_dir is not None, (
        "Please specify the wav2vec checkpoint directory."
    )
...
def _parse_args():
    ...
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        default="/root/autodl-tmp/Soul-AILab/SoulX-FlashTalk-14B",
        help="The path to FlashTalk model checkpoint directory.",
    )
    parser.add_argument(
        "--wav2vec_dir",
        type=str,
        default="/root/autodl-tmp/TencentGameMate/chinese-wav2vec2-base",
    ...
    parser.add_argument(
        "--cond_image",
        type=str,
        default="flashtalk_test_assets/1/image.jpeg",
        help="[meta file] The condition image path to generate the video.",
    )
    parser.add_argument(
        "--audio_path",
        type=str,
        default="flashtalk_test_assets/1/audio.wav",
...
def generate(args):
    ...
    human_speech_array_all, _ = librosa.load(
        args.audio_path, sr=infer_params["sample_rate"], mono=True
    )
    ...
    logger.info(f"Generate time: {generate_end_time - generate_start_time:.2f}s")
